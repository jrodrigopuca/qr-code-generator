/**
 * @fileoverview Tablas de campo de Galois GF(2^8) para Reed-Solomon
 * @description Tablas antilogarítmicas y logarítmicas para aritmética
 * de campos finitos usada en corrección de errores.
 * @module constants/galois
 * @see {@link https://www.thonky.com/qr-code-tutorial/error-correction-coding}
 */

/**
 * Tabla antilogarítmica (exponenciación) para GF(2^8).
 * antilog[i] = α^i donde α es el elemento primitivo.
 * Usa el polinomio primitivo x^8 + x^4 + x^3 + x^2 + 1.
 *
 * @example
 * ```typescript
 * // α^0 = 1, α^1 = 2, α^7 = 128, α^8 = 29 (overflow)
 * const alpha5 = GF_ANTILOG[5]; // 32
 * ```
 */
export const GF_ANTILOG: readonly number[] = [
...
/**
 * Tabla logarítmica para GF(2^8).
 * log[x] = i donde α^i = x.
 * log[0] es indefinido pero se incluye como 0 para conveniencia.
 *
 * @example
 * ```typescript
 * // log(1) = 0, log(2) = 1, log(4) = 2
 * const logOf29 = GF_LOG[29]; // 8 (porque α^8 = 29)
 * ```
 */
export const GF_LOG: readonly number[] = [
	0, // log(0) es indefinido, placeholder
	0,
	1,
	25,
	2,
